export default {
    nl : {
        types : "Team Types",
        no_types : "Er zijn geen team types gedefiniëerd",
        no_teams : "Er zijn geen teams gedefiniëerd",
        type_details : "Team Type Details",
        type : {
            create_title : "Maak Team Type",
            update_title : "Wijzig Team Type",
            details : "Team Type",
            form : {
                name : {
                    label : "Naam",
                    hint : "Naam van het type"
                },
                min_age : {
                    label : "Min. Leeftijd",
                    hint : "De minimum leeftijd voor dit team"
                },
                max_age : {
                    label : "Max. Leeftijd",
                    hint : "De maximum leeftijd voor dit team"
                },
                remark : {
                    label : "Opmerking",
                    hint : "Een opmerking over dit type."
                },
                gender : {
                    label : "Geslacht",
                    hint : "Zal het het team beperkt zijn tot een bepaald geslacht?"
                }
            }
        },
        active : "Actief",
        competition : "Wordt dit type gebruikt in competitie?",
        submit : "Verstuur",
        clear : "Maak Leeg",
        teams : "Teams",
        remark : 'Opmerking',
        no_season : "Geen Seizoen",
        no_type : "Geen Teamtype",
        no_restriction : "Geen beperking",
        not_active : "Niet actief",
        used_competition : "Gebruikt voor competitie",
        no_competition : "Niet gebruikt voor competitie",
        male : "Man",
        female : "Vrouw",
        team : {
            create_title : 'Maak nieuw team',
            update_title : 'Wijzig Team',
            details : "Team Details",
            form : {
                name : {
                    label : "Naam",
                    hint : "Naam van het team"
                },
                season : {
                    label : "Seizoen",
                    hint : "Een team dat niet gebonden is aan een seizoen kan gebruikt worden om leden te groeperen (bv. bij registratie trainingen). Een team dat bij een seizoen hoort kan gebruikt worden voor tornooien/wedstrijden."
                },
                team_type : {
                    label : "Teamtype",
                    hint : "Een type kan gebruikt worden om extra controles uit te voeren bij het toevoegen van leden (Bv. leeftijd)."
                },
                remark : {
                    label : "Remark",
                    hint : "A remark about this type."
                }
            }
        },
        members : "Leden",
        age : 'jaar',
        age_remark : 'Leeftijd is berekend op de einddatum van het seizoen {season} ({start} &ndash; {end})',
        no_members : 'Dit team heeft geen leden'
    },
    en : {
        types : "Team Types",
        no_types : "There are no team types defined yet",
        no_teams : "There are no teams defined yet",
        type_details : "Team Type Details",
        type : {
            type_create : "Create Team Type",
            type_update : "Update Team Type",
            details : "Team Type",
            form : {
                name : {
                    label : "Name",
                    hint : "Name of the type"
                },
                min_age : {
                    label : "Min. Age",
                    hint : "The minimum age of a member to be part of this team."
                },
                max_age : {
                    label : "Max. Age",
                    hint : "The maximum age of a member to be part of this team."
                },
                remark : {
                    label : "Remark",
                    hint : "A remark about this type."
                },
                gender : {
                    label : "Gender",
                    hint : "Will the team be restricted to a specific gender?"
                }
            }
        },
        active : "Active",
        competition : "Is this for a competition?",
        submit: "Submit",
        clear: "Clear",
        teams: "Teams",
        remark : 'Remark',
        no_season : "No Season",
        no_type : "No Teamtype",
        no_restriction : "No restriction",
        not_active : "Not active",
        used_competition : "Used for competition",
        no_competition : "Not used for competition",
        male : "Male",
        female : "Female",
        team : {
            create_title : 'Create new team',
            update_title : 'Edit Team',
            details : "Team Details",
            form : {
                name : {
                    label : "Name",
                    hint : "Name of the team"
                },
                season : {
                    label : "Season",
                    hint : "A team that is not related to a season can be used to group members (to register the presence on a training for example). A team that is related to a season can be used for tournaments/games."
                },
                team_type : {
                    label : "Teamtype",
                    hint : "A type can be used to add more checks when a member is added to the team (Age for example)."
                },
                remark : {
                    label : "Remark",
                    hint : "A remark about this type."
                }
            }
        },
        members : "Members",
        age : 'years',
        age_remark : 'Age is determined on the end date of the season {season} ({start} &ndash; {end})',
        no_members : "This team doesn't have members"
    }
};
